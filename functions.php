...
<?php
    $args = array(
      ...
      foreach ( $terms as $term ) { ?>
        <option value="<?php echo $term->slug; ?>" <?php if ( $_GET['schedule-cat'] == $term->slug ) { print 'selected'; } ?>><?php echo $term->name; ?></option>
<?php
      }
?>
    </select>
...
jQuery(function($) {
    if( $('#post_type').val() == 'artist' ||
        $('#post_type').val() == 'fc-entry' ) {
      if( $('#title').val() == '' ) {
        alert('タレント名を入力してください。');
        e.preventDefault();
      }
    }elseif( $('#post_type').val() == 'news' ||
            $('#post_type').val() == 'schedule' ) {
